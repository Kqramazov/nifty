...
def ssf_validation(model, x_1, edge_index_1, x_2, edge_index_2, y):
    ...
    l3 = F.binary_cross_entropy_with_logits(
        c1[idx_val], y[idx_val].unsqueeze(1).float().to(device))/2
    l4 = F.binary_cross_entropy_with_logits(
        c2[idx_val], y[idx_val].unsqueeze(1).float().to(device))/2
...
# Load pokec dataset
    ...
    test_idx = False
    sens_idx = 3
    adj, features, labels, idx_train, idx_val, idx_test, sens, edge_index = load_pokec(dataset,
                                                                                       sens_attr,
                                                                                       predict_attr,
                                                                                       path=path,
                                                                                       label_number=label_number,
...
# Load pokec dataset
    ...
    exit(0)
if args.dataset != 'pokec_n' and args.dataset != 'pokec_z':
...
if args.model == 'gcn':
    ...
    model = RobustGCN(nnodes=adj.shape[0], nfeat=features.shape[1], nhid=args.hidden,
                      nclass=num_class, dropout=args.dropout, device=device, seed=args.seed)

elif args.model == 'ssf':
    encoder = Encoder(
        in_channels=features.shape[1], out_channels=args.hidden, base_model=args.encoder).to(device)
    model = SSF(encoder=encoder, num_hidden=args.hidden, num_proj_hidden=args.proj_hidden,
                sim_coeff=args.sim_coeff, nclass=num_class).to(device)
    val_edge_index_1 = dropout_adj(
        edge_index.to(device), p=args.drop_edge_rate_1)[0]
    val_edge_index_2 = dropout_adj(
        edge_index.to(device), p=args.drop_edge_rate_2)[0]
    val_x_1 = drop_feature(features.to(
        device), args.drop_feature_rate_2, sens_idx, sens_flag=False)
    val_x_2 = drop_feature(features.to(
        device), args.drop_feature_rate_2, sens_idx)
    par_1 = list(model.encoder.parameters()) + list(model.fc1.parameters()) + list(
        model.fc2.parameters()) + list(model.fc3.parameters()) + list(model.fc4.parameters())
...
if args.model == 'rogcn':
    model.fit(features, adj, labels, idx_train, idx_val=idx_val,
              idx_test=idx_test, verbose=True, attention=False, train_iters=args.epochs)
...
for epoch in range(args.epochs+1):
    ...
    if args.model in ['gcn', 'sage', 'gin', 'jk', 'infomax']:
        ...
        auc_roc_train = roc_auc_score(labels.cpu().numpy(
        )[idx_train], output.detach().cpu().numpy()[idx_train])
        loss_train.backward()
        optimizer.step()
        ...
        loss_val = F.binary_cross_entropy_with_logits(
            output[idx_val], labels[idx_val].unsqueeze(1).float().to(device))
        acc_val = accuracy(output[idx_val], labels[idx_val])
        auc_roc_val = roc_auc_score(labels.cpu().numpy(
        )[idx_val], output.detach().cpu().numpy()[idx_val])
        f1_val = f1_score(labels[idx_val].cpu().numpy(),
                          preds[idx_val].cpu().numpy())
        ...
        for _ in range(rep):
            ...
            x_1 = drop_feature(
                features, args.drop_feature_rate_2, sens_idx, sens_flag=False)
        ...
        # Binary Cross-Entropy
        # print(c1[idx_train].shape)
        l3 = F.binary_cross_entropy_with_logits(
            c1[idx_train], labels[idx_train].unsqueeze(1).float().to(device))/2
        l4 = F.binary_cross_entropy_with_logits(
            c2[idx_train], labels[idx_train].unsqueeze(1).float().to(device))/2
        ...
        val_s_loss, val_c_loss = ssf_validation(
            model, val_x_1, val_edge_index_1, val_x_2, val_edge_index_2, labels)
...
if args.model in ['gcn', 'sage', 'gin', 'jk', 'infomax']:
    ...
    noisy_features = features.clone().to(
        'cpu') + torch.ones(features.shape).normal_(0, 1).to('cpu')
...
auc_roc_test = roc_auc_score(labels.cpu().numpy(
)[idx_test.cpu()], output.detach().cpu().numpy()[idx_test.cpu()])
counterfactual_fairness = 1 - \
    (output_preds.eq(counter_output_preds)[
     idx_test].sum().item()/idx_test.shape[0])
robustness_score = 1 - (output_preds.eq(noisy_output_preds)
                        [idx_test].sum().item()/idx_test.shape[0])

parity, equality = fair_metric(output_preds[idx_test].cpu(
).numpy(), labels[idx_test].cpu().numpy(), sens[idx_test].numpy())
f1_s = f1_score(labels[idx_test].cpu().numpy(),
...
stat = pd.DataFrame({'auc_roc': [auc_roc_test * 100],
                     'Acc': [acc_test.item() * 100],
                     'f1': [f1_s * 100],
                     'parity': [parity * 100],
                     'equality': [equality * 100]})

stat = pd.concat([stat, args_df], axis=1)
result_file = "nifty.csv"
if not os.path.exists(result_file):
    stat.to_csv(result_file, encoding='utf_8', mode='a+', index=False)
else:
    stat.to_csv(result_file, encoding='utf_8', mode='a+',
